...
class TestCalcSpeed:
    """calc_speed() adds per-point dist_m/speed_kmh columns."""
    ...
    def test_computes_distance_matching_haversine(self, points_df):
        ...
        expected_dist = EARTH_RADIUS_M * math.radians(1.0)
        assert result.loc[1, "dist_m"] == pytest.approx(expected_dist)
        assert result.loc[2, "dist_m"] == pytest.approx(expected_dist)
    ...
    def test_reuses_existing_dist_m_column_instead_of_recomputing(self, points_df):
        points_df["dist_m"] = [np.nan, 1234.0, 5678.0]
        ...
        assert result.loc[1, "dist_m"] == 1234.0
        assert result.loc[2, "dist_m"] == 5678.0
        # speed is derived from the *provided* dist_m, not the true
        # haversine distance (~111 km), which would give ~400 km/h instead.
        assert result.loc[1, "speed_kmh"] == pytest.approx(1234.0 / 1000 * 3.6)
