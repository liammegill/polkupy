"""Provides the :class:`Trips` class.

This class is a collection of rides, backed by one concatenated
:class:`pandas.DataFrame` grouped by ``ride_id``.
"""
...
class Trips:
    ...
    The class is backed by one :class:`pandas.DataFrame` and grouped by
    ``ride_id``.
    ...
    def __init__(self, data: pd.DataFrame):
        ...
        if missing:
            raise ValueError(
                "Trips data is missing required column(s): "
    ...
    def from_rides(cls, rides: Iterable[Ride | Trips]) -> Trips:
        """Concatenate several :class:`Ride`/:class:`Trips` into one collection.
    ...
    def __repr__(self) -> str:
        """Return a concise ``Trips(N rides, M points)`` summary."""
    ...
    def _repr_html_(self) -> str:
        """Render collection metadata plus a per-ride table.

        This allows a :class:`Trips` to display as a summary card just by being
        the last line of a Jupyter cell.
        """
        return (
            "<div><h3>Trips</h3>"
    ...
    def _repr_ride_table(self) -> str:
        """The per-ride ``<table>`` (ride ID, bike ID, point count).

        The table is capped at :attr:`_REPR_MAX_ROWS` rides with a "... and N
        more" footer.
        """
        has_bike_id = "bike_id" in self.data.columns
        ...
        if len(ride_ids) > self._REPR_MAX_ROWS:
            ...
            footer = (
                f'<p style="margin-top: 0.5em;">'
                f"<em>... and {remaining} more rides</em></p>"
            )
    ...
    def in_bbox(
        ...
        """Keep rides that pass through a geographic bounding box.

        See :func:`~polkupy.algorithms.filters.filter_rides_in_bbox`.
    ...
    def between(
        ...
        return self.filter(
            lambda ride: (
                ride.starts_near(*start, radius_km) and ride.ends_near(*end, radius_km)
            )
        )
